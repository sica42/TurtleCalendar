TurtleCalendar_translation = TurtleCalendar_translation or {}

TurtleCalendar_translation[ "zhCN" ] = {
	[ "TurtleCalendar" ] = "乌龟日历",
	[ "Turtle Calendar v" ] = "乌龟日历 v",
	[ 'YELL_TRIGGER_ALLIANCE_ONYXIA' ] = '可怕的奥妮克希亚悬挂在拱门上！',
	[ 'YELL_TRIGGER_ALLIANCE_NEFARIAN' ] = '联盟的市民们，黑石之主已死，奈法利安已被击败！',
	[ 'YELL_TRIGGER_HORDE_ONYXIA' ] = '奥妮克希亚已经被斩杀了',
	[ 'YELL_TRIGGER_HORDE_NEFARIAN' ] = '奈法利安已被击杀！奥格瑞玛的子民们，臣服于这股强大的力量吧！',
	[ "Onyxia" ] = "奥妮克希亚",
	[ "Nefarian" ] = "奈法利安",
	[ "Karazhan" ] = "卡拉赞",
	[ "N/A" ] = "无",
	[ "Global" ] = "全局",
	[ "Raids" ] = "团队副本",
	[ "Misc" ] = "杂项",
	[ "Condensed timers" ] = "精简计时器",
	[ "Show timers" ] = "显示计时器",
	[ "Show dates" ] = "显示日期",
	[ "Battleground" ] = "战场",
	[ "Darkmoon faire" ] = "暗月马戏团",
	[ "Instances" ] = "副本",
	[ "No lockouts" ] = "无锁定",
	[ "World Buffs" ] = "世界增益",
	[ "Stormwind buffs" ] = "暴风城增益",
	[ "Orgrimmar buffs" ] = "奥格瑞玛增益",
	[ "Weekly quest reminder" ] = "周常任务提醒",
	[ "Raid" ] = "团队",
	[ "Resets every %d days" ] = "每 %d 天重置一次",
	[ "Edge of Madness" ] = "疯狂之缘",
	[ "BG of the day" ] = "今日战场",
	[ "Coming next day: " ] = "明日预告：",
	[ "Thunder Bluff" ] = "雷霆崖",
	[ "Goldshire" ] = "闪金镇",
	[ "Day off" ] = "休息日",
	[ "Resets every day" ] = "每日重置",
	[ "It will be back tomorrow" ] = "明日返回",
	[ "Stays at %s until" ] = "保持在 %s 直到",
	[ "Darkmoon Faire position" ] = "暗月马戏团位置",
	[ "Moves out every Sunday" ] = "每周日出发",
	[ "days" ] = "天",
	[ "hours" ] = "小时",
	[ "min" ] = "分",
	[ "sec" ] = "秒",
	[ "Left-click to toggle." ] = "左键点击展开/收起",
	[ "Toggle the calendar" ] = "切换日历",
	[ "Alterac Valley" ] = "奥特兰克山谷",
	[ "Warsong Gulch" ] = "战歌峡谷",
	[ "Arathi Basin" ] = "阿拉希盆地",
	[ "Blood Ring Arena" ] = "血环竞技场",
	[ "Sunnyglade Valley" ] = "阳光林地",
	[ "Gri'lek" ] = "格里雷克",
	[ "Hazza'rah" ] = "哈扎拉尔",
	[ "Renataki" ] = "雷纳塔基",
	[ "Wushoolay" ] = "乌苏雷",
	[ "Molten Core" ] = "熔火之心",
	[ "Blackwing Lair" ] = "黑翼之巢",
	[ "Ahn'Qiraj Temple" ] = "安其拉神殿",
	[ "Naxxramas" ] = "纳克萨玛斯",
	[ "Emerald Sanctum" ] = "翡翠圣殿",
	[ "Onyxia's Lair" ] = "奥妮克希亚的巢穴",
	[ "Lower Karazhan Halls" ] = "卡拉赞下层大厅",
	[ "Zul'Gurub" ] = "祖尔格拉布",
	[ "Ruins of Ahn'Qiraj" ] = "安其拉废墟",
	[ "Powered by " ] = "技术支持：",
	[ "ID" ] = "编号",
}
